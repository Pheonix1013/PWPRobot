...
def detect_horizontal_end_line(lines, h):
    if lines is None:
        return False
    ...
    for l in lines:
        ...
        if slope < 0.2:
            ...
            # Check if near bottom (end of path)
            if y_avg > h * 0.8:
                # Optional: ignore tiny lines
                length = ((x2 - x1)**2 + (y2 - y1)**2)**0.5
                if length > 80:
                    return True
...
def process_frame(frame):
    ...
    horizontal_line_detected = detect_horizontal_end_line(lines, h)
    ...
    if horizontal_line_detected:
        auto_command = "left"
        lane_status = "turning left"

    elif stop_detected:
        auto_command = "stop"
        if autonomous_enabled:
            autonomous_enabled = False
            lane_status = "automation stopped"
        else:
            lane_status = "Stop line detected"
